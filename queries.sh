...
echo "$($PSQL "SELECT name FROM teams WHERE team_id IN (SELECT DISTINCT(winner_id) FROM games WHERE year = 2014 AND round = 'Eighth-Final') OR team_id IN (SELECT DISTINCT(opponent_id) FROM games WHERE year = 2014 AND round = 'Eighth-Final') ORDER BY name")"
